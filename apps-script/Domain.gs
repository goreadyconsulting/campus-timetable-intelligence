...
var CTIDomain = (() => {
  ...
  __export(domain_exports, {
    ...
    assertArchive: () => assertArchive,
    assertRecord: () => assertRecord,
    detectOccurrenceConflicts: () => detectOccurrenceConflicts,
    entities: () => entities,
    makePublication: () => makePublication,
    migrateData: () => migrateData,
    publicationReadiness: () => publicationReadiness,
    publishedVariantChanges: () => publishedVariantChanges,
  ...
  function gregorianToWeek(gregObj, minDaysInFirstWeek = 4, startOfWeek = 1) {
    const { year, month, day } = gregObj, ordinal = computeOrdinal(year, month, day), weekday2 = isoWeekdayToLocal(dayOfWeek(year, month, day), startOfWeek);
  ...
  function formatOffset(offset2, format) {
    const hours = Math.trunc(Math.abs(offset2 / 60)), minutes2 = Math.trunc(Math.abs(offset2 % 60)), sign = offset2 >= 0 ? "+" : "-";
    switch (format) {
      case "short":
        return `${sign}${padStart(hours, 2)}:${padStart(minutes2, 2)}`;
      case "narrow":
        return `${sign}${hours}${minutes2 > 0 ? `:${minutes2}` : ""}`;
      case "techie":
        return `${sign}${padStart(hours, 2)}${padStart(minutes2, 2)}`;
      default:
  ...
  var Formatter = class _Formatter {
    ...
    formatDateTimeFromString(dt, fmt) {
      ...
      }, meridiem = () => knownEnglish ? meridiemForDateTime(dt) : string({ hour: "numeric", hourCycle: "h12" }, "dayperiod"), month = (length, standalone) => knownEnglish ? monthForDateTime(dt, length) : string(standalone ? { month: length } : { month: length, day: "numeric" }, "month"), weekday2 = (length, standalone) => knownEnglish ? weekdayForDateTime(dt, length) : string(
  ...
  var validDate = (v) => /^\d{4}-\d{2}-\d{2}$/.test(v) && DateTime.fromISO(v, { zone: "UTC" }).isValid;
  var validTime = (v) => /^([01]\d|2[0-3]):[0-5]\d$/.test(v);
  var validZone = (v) => IANAZone.isValidZone(v);
  var datePlus = (v, n2) => DateTime.fromISO(v, { zone: "UTC" }).plus({ days: n2 }).toISODate();
  var monday = (v) => DateTime.fromISO(v, { zone: "UTC" }).startOf("week").toISODate();
  var weekday = (v) => DateTime.fromISO(v, { zone: "UTC" }).weekday;
  function toUtc(date, time, zone) {
    ...
    if (!local.isValid || local.toFormat("yyyy-MM-dd HH:mm") !== `${date} ${time}`)
      throw new Error(
  ...
  function calendarForYear(n2 = 2026) {
    const year = {
      id: `AY-${n2}`,
      name: `${n2}/${String(n2 + 1).slice(-2)}`,
      startDate: `${n2}-09-01`,
      endDate: `${n2 + 1}-08-31`,
      revision: 1
    };
    const first = monday(`${n2}-09-21`);
    const terms = [
      ["AUT", "Autumn", 0, 90],
      ["SPR", "Spring", 112, 202],
      ["SUM", "Summer", 224, 286]
    ...
    const weeks = [];
    for (let i = 0; i < 50; i++) {
      const startDate = datePlus(first, i * 7), endDate = datePlus(startDate, 6);
      if (endDate > year.endDate) break;
      const term = terms.find(
        (t) => startDate >= t.startDate && startDate <= t.endDate
      );
      const reading = [6, 22].includes(i), exams = i >= 37 && i <= 40;
      weeks.push({
        id: `${year.id}-W${i + 1}`,
        academicYearId: year.id,
        termId: term?.id,
        ...
        label: reading ? "Reading week" : exams ? "Examinations" : term ? "Teaching" : "Holiday",
        revision: 1
      });
    }
  ...
  function normaliseCampusData(input) {
    const rooms = input.rooms;
    const modules = input.modules.map((module) => ({
      ...module,
      campus: module.campus || campusFromModule(module, input),
      programmeId: module.programmeId || programmeId(
        module.campus || campusFromModule(module, input),
        module.course
      )
    }));
    const programmes = input.programmes !== void 0 ? input.programmes : deriveProgrammes({ ...input, rooms, modules });
    const lecturers = input.lecturers.map(
      (lecturer) => normaliseLecturer(lecturer, modules)
    );
    const students = input.students !== void 0 ? input.students.map((student) => ({
      ...student,
      moduleCodes: student.moduleCodes || []
    })) : deriveLegacyStudents({ ...input, rooms, modules, programmes });
    const sessions = input.sessions.map((session) => ({
      ...session,
      studentIds: session.studentIds ?? students.filter(
        (s2) => s2.campus === session.campus && s2.moduleCodes.includes(session.moduleCode)
      ).map((s2) => s2.id)
    }));
    return {
      ...input,
  ...
  function campusFromModule(module, data) {
    ...
    const group = data.studentGroups.find(
      (item) => item.name === module.studentGroup
    );
    if (group?.campus) return group.campus;
    const lecturer = data.lecturers.find(
      (item) => item.id === module.lecturerId || item.name === module.lecturerName
    );
    return lecturer?.primaryCampus || lecturer?.preferredCampus || "Birmingham";
  }
  function programmeId(campus, programme) {
  ...
  function staffCanTeachAtCampus(lecturer, campus) {
  ...
  function normaliseLecturer(lecturer, modules) {
    const assignedCampuses = Array.from(
      new Set(
        modules.filter(
          (module) => module.lecturerId === lecturer.id || module.lecturerName === lecturer.name || lecturer.modules.includes(module.code)
        ).map((module) => module.campus).filter(Boolean)
      )
    );
    const primaryCampus = lecturer.primaryCampus || lecturer.preferredCampus || assignedCampuses[0] || "Birmingham";
    const additionalCampuses = lecturer.additionalCampuses !== void 0 ? Array.from(
      new Set(
        lecturer.additionalCampuses.filter(
          (campus) => campus !== primaryCampus
        )
      )
    ) : [];
    return {
      ...lecturer,
      primaryCampus,
      preferredCampus: lecturer.preferredCampus || primaryCampus,
      additionalCampuses
    };
  }
  function deriveProgrammes(data) {
    const keys = /* @__PURE__ */ new Map();
    const add = (campus, course) => {
    ...
    data.modules.forEach(
      (module) => add(module.campus || campusFromModule(module, data), module.course)
    );
    return Array.from(keys.values()).sort(
      (a, b) => `${a.campus}-${a.code}`.localeCompare(`${b.campus}-${b.code}`)
    );
  }
  function deriveLegacyStudents(data) {
    const students = [];
    let counter = 1;
    data.studentGroups.forEach((group) => {
      const groupModules = data.modules.filter(
        (module) => module.studentGroup === group.name || module.course === group.course && module.campus === group.campus
      );
      const programme = data.programmes?.find(
        (item) => item.campus === group.campus && item.code === group.course
      );
  ...
  // lib/workflow.ts
  function createTemplatesFromData(data) {
    return data.modules.map(
      (module, index) => createTemplate(module, data, index)
    );
  }
  function validateTemplate(template) {
    ...
    return [
      ...
      },
      {
        ...
        passed: Boolean((template.studentIds || []).length),
  ...
  function createTemplate(module, data, index) {
    const requirement = data.requirements.find(
      (item) => item.moduleCode === module.code
    );
    const group = data.studentGroups.find(
      (item) => item.name === (module.studentGroup || requirement?.studentGroup)
    ) || data.studentGroups.find(
      (item) => item.course === module.course && (!module.campus || item.campus === module.campus)
    );
    const lecturer = data.lecturers.find(
      (item) => item.id === module.lecturerId || item.name === module.lecturerName || item.modules.includes(module.code)
    );
    const activityType = activityTypeFromRoom(
      module.roomTypeRequired || requirement?.requiredRoomType || "Teaching room"
    );
    const campus = module.campus || group?.campus || lecturer?.primaryCampus || lecturer?.preferredCampus || "Birmingham";
    const students = (data.students || []).filter(
      (student) => student.campus === campus && student.moduleCodes.includes(module.code)
    );
    const programme = data.programmes?.find((item) => item.id === module.programmeId) || data.programmes?.find(
      (item) => item.campus === campus && item.code === module.course
    );
    const template = {
      ...
      moduleId: module.id,
      teachingWeeks: (data.teachingWeeks || []).filter(
        (w) => w.isTeachingWeek && w.academicYearId === data.academicYears?.[0]?.id
      ).map((w) => w.weekNumber),
      studentIds: students.map((student) => student.id),
      studentGroup: group?.name || module.studentGroup || requirement?.studentGroup || "",
      lecturerSuitability: lecturer ? lecturer.name : "",
      roomSuitability: module.roomTypeRequired || requirement?.requiredRoomType || "Workshop room",
  ...
  function activityTypeFromRoom(value) {
  ...
  function migrateData(input) {
    ...
    const data = normaliseCampusData(input), cal = calendarForYear();
    const names = [
      ...new Set(
        [
    ...
    data.campuses ?? (data.campuses = names.map((name) => ({
      id: name === "Birmingham" ? "BHM" : name === "Manchester" ? "MAN" : `CAM-${name.replace(/\W/g, "").toUpperCase()}`,
    ...
    data.academicYears ?? (data.academicYears = [cal.year]);
    data.terms ?? (data.terms = cal.terms);
    data.teachingWeeks ?? (data.teachingWeeks = cal.weeks);
    ...
    data.lecturers = data.lecturers.map((l2) => ({
      ...l2,
      id: l2.id || `STA-${l2.name}`,
      revision: l2.revision || 1
    }));
    data.templates ?? (data.templates = createTemplatesFromData(data));
    data.templates = data.templates.map((t) => ({
      ...t,
      moduleId: t.moduleId || data.modules.find((m) => m.code === t.moduleCode && m.campus === t.campus)?.id,
      revision: t.revision || 1,
      academicYearId: t.academicYearId || cal.year.id,
      teachingWeeks: t.teachingWeeks.filter(
        (n2) => data.teachingWeeks.some(
          (w) => w.academicYearId === (t.academicYearId || cal.year.id) && w.weekNumber === n2 && w.isTeachingWeek
        )
      )
    }));
    data.exceptions ?? (data.exceptions = []);
    data.variants ?? (data.variants = []);
    data.availabilityRules ?? (data.availabilityRules = []);
    data.travelRules ?? (data.travelRules = []);
    data.conflictReviews ?? (data.conflictReviews = []);
    ...
    data.series ?? (data.series = data.sessions.map(
      (s2) => seriesFromSession(data, s2, `SER-${s2.id}`)
    ));
    data.allocations ?? (data.allocations = (data.students || []).flatMap(
      (s2) => data.modules.filter((m) => m.campus === s2.campus && s2.moduleCodes.includes(m.code)).map((m) => ({
        id: `ALLOC-${s2.id}-${m.id}`,
        studentId: s2.id,
        moduleId: m.id,
    ...
    data.dataRevision ?? (data.dataRevision = 1);
    return data;
  }
  function seriesFromSession(data, session, id = "") {
    const module = data.modules.find(
      (m) => m.code === session.moduleCode && m.campus === session.campus
    ), campus = data.campuses?.find((c) => c.name === session.campus), template = data.templates?.find(
      (t) => t.moduleCode === session.moduleCode && t.campus === session.campus && !t.archived
    );
    const year = data.academicYears?.find(
      (y) => session.date ? session.date >= y.startDate && session.date <= y.endDate : y.id === template?.academicYearId
    ...
    return {
      ...
      academicYearId: year.id,
      dayOfWeek: session.date ? weekday(session.date) : days.indexOf(session.day) + 1,
      ...
      teachingWeeks: session.date ? weeks.filter(
        (w) => session.date >= w.startDate && session.date <= w.endDate
      ).map((w) => w.weekNumber) : template?.teachingWeeks || weeks.map((w) => w.weekNumber),
      staffIds: data.lecturers.filter((l2) => l2.name === session.lecturer).map((l2) => l2.id),
      locationId: data.rooms.find(
  ...
  function baseDates(series, weeks) {
    return weeks.filter(
      (w) => !w.archived && w.academicYearId === series.academicYearId && w.isTeachingWeek && series.teachingWeeks.includes(w.weekNumber)
    ).map((week) => ({
      week,
      date: datePlus(monday(week.startDate), series.dayOfWeek - 1)
    })).filter((d) => d.date >= series.startDate && d.date <= series.endDate);
  }
  function variantApplies(v, date, week) {
  ...
  function variantDates(s2, v, weeks) {
    return baseDates(s2, weeks).filter(
      (d) => variantApplies(v, d.date, d.week.weekNumber)
    );
  }
  function assertNoVariantOverlap(s2, v, data) {
    const dates = variantDates(s2, v, data.teachingWeeks || []);
    if (!dates.length)
    ...
    const overlap = data.variants?.find(
      (a) => a.id !== v.id && a.seriesId === s2.id && !a.archived && a.operation !== "additional_session" && priorities[a.scope] === priorities[v.scope] && dates.some((d) => variantApplies(a, d.date, d.week.weekNumber))
    );
    if (overlap)
      throw new Error(
  ...
  function resolveOccurrences(data, start, end, includeCancelled = false) {
    const result = [];
    ...
    for (const s2 of data.series || []) {
      if (s2.archived) continue;
      const campus = data.campuses?.find((c) => c.id === s2.campusId), module = data.modules.find((m) => m.id === s2.moduleId), programme = data.programmes?.find((p) => p.id === s2.programmeId);
      for (const { week, date } of baseDates(s2, data.teachingWeeks || []).filter(
        (d) => d.date >= datePlus(start, -7) && d.date <= datePlus(end, 7)
      )) {
        const variants = (grouped.get(s2.id) || []).filter(
          (v) => variantApplies(v, date, week.weekNumber)
        );
        const overrides = variants.filter((v) => v.operation !== "additional_session").sort(
          (a, b) => priorities[b.scope] - priorities[a.scope] || a.id.localeCompare(b.id)
        );
        if (overrides.length > 1 && priorities[overrides[0].scope] === priorities[overrides[1].scope])
          throw new Error(
            `Overlapping variants require review for ${module?.code || s2.id}, Week ${week.weekNumber}.`
          );
        for (const v of [
          overrides[0],
          ...variants.filter((v2) => v2.operation === "additional_session")
        ]) {
          const effective = { ...s2, ...v?.patch };
          const cancelled = s2.status === "Cancelled" || v?.operation === "cancel";
          ...
          const room = data.rooms.find((r) => r.id === effective.locationId), staff = data.lecturers.filter(
            (l2) => effective.staffIds.includes(l2.id || "")
          );
          result.push({
            id: `${s2.id}@${date}${v?.operation === "additional_session" ? `+${v.id}` : ""}`,
            seriesId: s2.id,
            variantId: v?.id,
            ...
            academicYearId: s2.academicYearId,
            campusId: s2.campusId,
            campus: campus?.name || s2.campusId,
            moduleId: s2.moduleId,
            moduleCode: module?.code || s2.moduleId,
            moduleName: module?.name || s2.moduleId,
            ...
            startAtUtc: toUtc(dateCurrent, effective.startLocalTime, s2.timeZone),
            endAtUtc: toUtc(dateCurrent, effective.endLocalTime, s2.timeZone),
            staffIds: effective.staffIds,
            lecturer: staff.map((l2) => l2.name).join(", "),
            locationId: effective.locationId,
            room: room?.room || effective.locationId,
            studentIds: effective.studentIds,
            enrolled: Math.max(s2.plannedSize, effective.studentIds.length),
            capacity: room?.capacity || 0,
            recurring: s2.startDate !== s2.endDate,
            status: cancelled ? "Cancelled" : v?.status || s2.status,
            base: {
              dayOfWeek: s2.dayOfWeek,
              startLocalTime: s2.startLocalTime,
              endLocalTime: s2.endLocalTime,
              locationId: s2.locationId,
              staffIds: s2.staffIds,
              studentIds: s2.studentIds
            }
          });
        }
      }
    ...
    return result.sort(
      (a, b) => a.startAtUtc.localeCompare(b.startAtUtc) || a.id.localeCompare(b.id)
    );
  }
  ...
  function records(data, entity) {
    return data[entity] || [];
  }
  function requireValue(test, message) {
    if (!test) throw new Error(message);
  }
  function assertRecord(data, entity, r) {
    const ref = (e, id) => records(data, e).find((x) => x.id === id && !x.archived);
    const campus = (name) => records(data, "campuses").find(
      (x) => x.name === name && x.active && !x.archived
    );
    const range = () => requireValue(
      validDate(r.startDate) && validDate(r.endDate) && r.startDate <= r.endDate,
      "Enter a valid ordered date range."
    );
    const times = (a, b) => requireValue(
      validTime(a) && validTime(b) && a < b,
      "End time must be later than start time."
    );
    requireValue(r.id, "A permanent record ID is required.");
    if (r.archived) return;
    if (["rooms", "students", "programmes", "modules", "templates"].includes(entity))
    ...
    if (entity === "rooms") {
      ...
      requireValue(
        !records(data, entity).some(
          (x) => x.id !== r.id && !x.archived && x.campus === r.campus && x.room === r.room
        ),
    ...
    if (entity === "lecturers") {
      ...
      requireValue(
        Array.isArray(r.additionalCampuses) && r.additionalCampuses.every(
          (c) => campus(c) && c !== r.primaryCampus
        ),
    ...
    if (entity === "programmes" || entity === "modules") {
      ...
      requireValue(
        !records(data, entity).some(
          (x) => x.id !== r.id && !x.archived && x.code === r.code && x.campus === r.campus
        ),
    ...
    if (entity === "students")
      requireValue(
        Array.isArray(r.moduleCodes) && r.moduleCodes.every(
          (code) => data.modules.some(
            (m) => !m.archived && m.campus === r.campus && m.programmeId === r.programmeId && m.code === code
          )
        ),
        "Student modules must belong to the selected programme and campus."
      );
    if (entity === "academicYears" || entity === "terms" || entity === "teachingWeeks") {
      range();
      ...
      if (entity === "teachingWeeks") {
        requireValue(
          Number.isInteger(r.weekNumber) && r.weekNumber > 0 && r.weekNumber <= 53,
        ...
        requireValue(
          !records(data, entity).some(
            (w) => w.id !== r.id && !w.archived && w.academicYearId === r.academicYearId && (w.weekNumber === r.weekNumber || w.startDate <= r.endDate && w.endDate >= r.startDate)
          ),
    ...
    if (entity === "series") {
      ...
      const c = ref("campuses", r.campusId), m = ref("modules", r.moduleId), p = ref("programmes", r.programmeId), y = ref("academicYears", r.academicYearId);
      requireValue(
        c && m && p && m.campus === c.name && p.campus === c.name && m.programmeId === p.id,
      ...
      requireValue(
        Array.isArray(r.teachingWeeks) && r.teachingWeeks.length && r.teachingWeeks.every(
          (n2) => data.teachingWeeks?.some(
            (w) => !w.archived && w.academicYearId === r.academicYearId && w.weekNumber === n2 && w.isTeachingWeek
          )
        ),
        "Select actual teaching weeks."
      );
      requireValue(
        ref("rooms", r.locationId) && Array.isArray(r.staffIds) && r.staffIds.length && r.staffIds.every((id) => ref("lecturers", id)),
        "Select a location and staff."
      );
      requireValue(
        Array.isArray(r.studentIds) && r.studentIds.length && r.studentIds.every((id) => ref("students", id)),
    ...
    if (entity === "variants") {
      const s2 = ref("series", r.seriesId);
      requireValue(s2, "Series is missing or archived.");
      requireValue(
        [
          "single_occurrence",
          "week_set",
      ...
      requireValue(
        r.patch && Object.keys(r.patch).every(
          (k) => [
            "dayOfWeek",
            "startLocalTime",
            "endLocalTime",
      ...
      if (r.scope === "week_set")
        requireValue(
          r.teachingWeeks?.length && r.teachingWeeks.every((n2) => s2.teachingWeeks.includes(n2)),
          "Select weeks in the series."
        );
      const merged = { ...s2, ...r.patch };
      ...
      requireValue(
        merged.staffIds.length && merged.staffIds.every((id) => ref("lecturers", id)),
        "Invalid staff."
      );
      requireValue(
        merged.studentIds.length && merged.studentIds.every((id) => ref("students", id)),
        "Invalid students."
      );
      assertNoVariantOverlap(s2, r, data);
      resolveOccurrences(
        {
          ...data,
    ...
    if (entity === "templates") {
      ...
      requireValue(
        r.durationHours > 0 && r.durationHours <= 12 && Number.isInteger(r.weeklySessions) && r.weeklySessions > 0 && r.plannedSize > 0,
        "Enter duration, weekly sessions and size."
      );
      requireValue(
        r.teachingWeeks?.length && r.teachingWeeks.every(
          (n2) => data.teachingWeeks?.some(
            (w) => !w.archived && w.academicYearId === r.academicYearId && w.weekNumber === n2 && w.isTeachingWeek
          )
        ),
      ...
      requireValue(
        r.studentIds?.length && r.studentIds.every(
          (id) => ref("students", id)?.campus === r.campus
        ),
        "Choose individual students on this campus."
      );
    }
    if (entity === "exceptions" || entity === "availabilityRules") {
      const e = r.resourceType === "Lecturer" ? "lecturers" : r.resourceType === "Room" ? "rooms" : r.resourceType === "Student" ? "students" : null;
      ...
      const rule = data.availabilityRules?.find(
        (x) => !x.archived && x.resourceId === r.resourceId && x.resourceType === r.resourceType
      );
      ...
      if (entity === "exceptions") {
        ...
        requireValue(
          !records(data, entity).some(
            (x) => x.id !== r.id && !x.archived && x.resourceType === r.resourceType && x.resourceId === r.resourceId
          ),
        ...
        r.days.forEach((d) => {
          ...
          let end = "";
          [...d.windows].sort((a, b) => a.start.localeCompare(b.start)).forEach((w) => {
            times(w.start, w.end);
            requireValue(
              !end || w.start >= end,
    ...
    if (entity === "allocations") {
      range();
      const s2 = ref("students", r.studentId), m = ref("modules", r.moduleId);
      requireValue(
        s2 && m && s2.campus === m.campus && s2.programmeId === m.programmeId,
        "Student and module must share a programme and campus."
      );
    }
    if (entity === "travelRules")
      requireValue(
        ref("campuses", r.fromCampusId) && ref("campuses", r.toCampusId) && r.fromCampusId !== r.toCampusId && r.minimumMinutes >= 0,
  ...
  function assertArchive(data, entity, id) {
    const active = (e) => records(data, e).filter((x) => !x.archived);
    const field = {
      rooms: "locationId",
      modules: "moduleId",
    ...
    const references = (r) => entity === "lecturers" ? r.staffIds?.includes(id) : entity === "students" ? r.studentIds?.includes(id) : field[entity] ? r[field[entity]] == id : false;
    if (active("series").some(references) || active("variants").some((v) => references(v.patch)))
      throw new Error(
    ...
    const used = entity === "programmes" ? active("modules").some((m) => m.programmeId === id) || active("students").some((s2) => s2.programmeId === id) : entity === "modules" ? active("templates").some((t) => t.moduleId === id) || active("allocations").some((a) => a.moduleId === id) : entity === "campuses" ? ["rooms", "modules", "programmes", "students", "lecturers"].some(
      (e) => active(e).some(
        (r) => r.campus === own?.name || r.primaryCampus === own?.name || r.additionalCampuses?.includes(own?.name)
      )
    ) : entity === "academicYears" ? active("terms").some((t) => t.academicYearId === id) || active("teachingWeeks").some((w) => w.academicYearId === id) : entity === "terms" ? active("teachingWeeks").some((w) => w.termId === id) : false;
    if (used)
      throw new Error(
  ...
  function roomTypeMatches(actual, required = "") {
    const clean = (s2) => s2.toLowerCase().replace(/^rm-/, "").replace(/classroom|room|teaching|\W/g, "");
    const a = clean(actual), b = clean(required);
    return !b || a === b || Boolean(a) && (a.includes(b) || b.includes(a)) || b === "largegroup" && /lecture|largegroup/.test(a) || b === "advocacy" && /moot|advocacy/.test(a);
  }
  function legacyWindows(staff, day) {
    const text = (staff.availability || "").toLowerCase(), names = ["mon", "tue", "wed", "thu", "fri", "sat", "sun"];
    const r = text.match(
      /(mon|tue|wed|thu|fri|sat|sun)\s*[-–]\s*(mon|tue|wed|thu|fri|sat|sun)/
    );
    const allowed = r ? names.map((_, i) => i + 1).filter(
      (i) => i >= names.indexOf(r[1]) + 1 && i <= names.indexOf(r[2]) + 1
    ) : names.flatMap((n2, i) => text.includes(n2) ? [i + 1] : []);
    if (allowed.length && !allowed.includes(day)) return [];
    const t = text.match(/(\d{2}:\d{2})\s*[-–]\s*(\d{2}:\d{2})/);
    return [{ start: t?.[1] || "08:00", end: t?.[2] || "20:00" }];
  }
  function resourceAvailable(data, o, type, id) {
    const rule = data.availabilityRules?.find(
      (r) => !r.archived && r.resourceType === type && r.resourceId === id
    ), staff = type === "Lecturer" ? data.lecturers.find((l2) => l2.id === id) : void 0;
    const campus = type === "Room" ? data.rooms.find((r) => r.id === id)?.campus : type === "Student" ? data.students?.find((s2) => s2.id === id)?.campus : staff?.primaryCampus;
    const zone = rule?.timeZone || data.campuses?.find((c) => c.name === campus)?.timeZone || o.timeZone;
    const a = DateTime.fromISO(o.startAtUtc, { zone }), b = DateTime.fromISO(o.endAtUtc, { zone }), date = a.toISODate(), start = a.toFormat("HH:mm"), end = b.toFormat("HH:mm");
    const exceptions = (data.exceptions || []).filter(
      (e) => !e.archived && e.resourceType === type && e.resourceId === id && date >= e.startDate && date <= e.endDate
    );
    if (exceptions.some(
      (e) => e.availabilityType === "Unavailable" && start < e.endTime && e.startTime < end
    ))
      return false;
    const available = exceptions.filter(
      (e) => e.availabilityType === "Available"
    );
    const windows = available.length ? available.map((e) => ({ start: e.startTime, end: e.endTime })) : rule ? rule.days.find((d) => d.dayOfWeek === a.weekday)?.windows || [] : staff ? legacyWindows(staff, a.weekday) : [{ start: "00:00", end: "23:59" }];
    return a.toISODate() === b.toISODate() && windows.some((w) => start >= w.start && end <= w.end);
  }
  function detectOccurrenceConflicts(data, occurrences) {
    const active = occurrences.filter((o) => o.status !== "Cancelled").sort((a, b) => a.startAtUtc.localeCompare(b.startAtUtc)), conflicts = [];
    function add(type, items, description, severity = "Critical", students = []) {
      const ids = items.map((o) => o.id).sort(), fingerprint = `${type}:${ids.join("|")}`, review = data.conflictReviews?.find(
    ...
    for (const o of active) {
      const room = data.rooms.find((r) => r.id === o.locationId), module = data.modules.find((m) => m.id === o.moduleId), campus = data.campuses?.find((c) => c.id === o.campusId), series = data.series?.find((s2) => s2.id === o.seriesId), template = data.templates?.find(
      ...
      if (o.start >= o.end || o.start < (campus?.openingTime || "08:00") || o.end > (campus?.closingTime || "20:00"))
        add(
      ...
      for (const id of o.staffIds || []) {
        const staff = data.lecturers.find((l2) => l2.id === id);
        ...
        else if (template?.lecturerSuitability && !staff.modules.includes(o.moduleCode) && module?.lecturerId !== id)
          add(
            "Staff suitability",
            [o],
      ...
      const invalid = (o.studentIds || []).filter(
        (id) => !data.students?.some(
          (s2) => s2.id === id && !s2.archived && s2.status === "Active" && s2.campus === o.campus && s2.programmeId === series?.programmeId && ((data.allocations || []).some(
            (a) => !a.archived && a.studentId === id && a.moduleId === o.moduleId
          ) ? (data.allocations || []).some(
            (a) => !a.archived && a.studentId === id && a.moduleId === o.moduleId && a.status === "Active" && a.startDate <= o.date && a.endDate >= o.date
          ) : s2.moduleCodes.includes(o.moduleCode))
        )
      );
      if (invalid.length)
        add(
    ...
    for (const [key, items] of weekly) {
      const staff = data.lecturers.find((l2) => l2.id === key.split("|")[0]);
      const hours = items.reduce(
        (n2, o) => n2 + (minutes(o.end) - minutes(o.start)) / 60,
        0
      );
    ...
    const maxTravel = Math.max(
      0,
      ...(data.travelRules || []).filter((r) => !r.archived).map((r) => r.minimumMinutes)
    );
    for (let i = 0; i < active.length; i++) {
      const a = active[i], end = Date.parse(a.endAtUtc);
      for (let j = i + 1; j < active.length; j++) {
        const b = active[j], start = Date.parse(b.startAtUtc);
        if (start >= end + maxTravel * 6e4) break;
        const staff = (a.staffIds || []).filter((id) => b.staffIds?.includes(id)), students = (a.studentIds || []).filter(
          (id) => b.studentIds?.includes(id)
        );
        if (a.startAtUtc < b.endAtUtc && b.startAtUtc < a.endAtUtc) {
          ...
          const rule = data.travelRules?.find(
            (r) => !r.archived && r.fromCampusId === a.campusId && r.toCampusId === b.campusId
          );
          if (rule && (start - end) / 6e4 < rule.minimumMinutes)
            add(
              "Campus travel time",
              [a, b],
  ...
  // lib/domain.ts
  function applyMutations(source, mutations, expected, requestId, uuid, now2) {
    ...
    const remap = (v) => typeof v === "string" ? ids[v] || v : Array.isArray(v) ? v.map(remap) : v && typeof v === "object" ? Object.fromEntries(Object.entries(v).map(([k, x]) => [k, remap(x)])) : v;
    const changes = [];
    for (const mutation of mutations) {
      const m = remap(mutation);
      if (!entities.includes(m.entity) || !["create", "update", "archive", "delete"].includes(m.operation))
        throw new Error("Unsupported mutation.");
      const rows = records(data, m.entity), old = rows.find((r) => r.id === m.id);
      if (m.operation !== "create" && (!old || old.revision !== m.expectedRevision))
        throw new Error("STALE: Record changed. Refresh before saving.");
      if (m.operation === "delete" && !["variants", "exceptions"].includes(m.entity))
      ...
      const record = {
        ...
        updatedAt: now2,
        ...m.operation === "archive" || m.operation === "delete" ? { archived: true } : {},
        ...m.entity === "variants" ? { status: "Draft" } : {}
      };
      record.createdAt = old?.createdAt || now2;
      if (m.entity === "campuses" && old && (old.name !== record.name || old.timeZone !== record.timeZone))
    ...
    for (const change of [...changes]) {
      ...
      for (const allocation of data.allocations || []) {
        ...
        if (!allocation.archived && allocation.studentId === student.id && (!module || module.campus !== student.campus || !student.moduleCodes.includes(module.code))) {
          const updated = {
            ...allocation,
            archived: true,
          ...
          data.allocations = data.allocations.map(
            (a) => a.id === allocation.id ? updated : a
          );
          changes.push({
            entity: "allocations",
            record: updated,
      ...
      for (const code of student.moduleCodes) {
        if (beforeCodes.includes(code) && change.before?.campus === student.campus)
          continue;
        const module = data.modules.find(
        ...
        if (data.allocations?.some(
          (a) => !a.archived && a.studentId === student.id && a.moduleId === module.id
        ))
          continue;
        const allocation = {
          id: uuid(),
    ...
    if (changes.some(
      ...
      for (const v of data.variants || [])
        if (!v.archived && !data.series?.find((s2) => s2.id === v.seriesId)?.archived)
  ...
  function publicationReadiness(data, start, end, campusIds, academicYearId) {
    if (!validDate(start) || !validDate(end) || start > end || end > datePlus(start, 370))
    ...
    if (!campusIds.length || campusIds.some(
      (id) => !data.campuses?.some((c) => c.id === id && !c.archived && c.active)
    ))
    ...
    const sessions = all.filter(
      (s2) => s2.date >= start && s2.date <= end && s2.academicYearId === academicYearId && campusIds.includes(s2.campusId)
    );
    const selected = new Set(sessions.map((s2) => s2.id));
    const conflicts = detectOccurrenceConflicts(data, all).filter(
      (c) => c.occurrenceIds?.some((id) => selected.has(id)) && ["Critical", "High"].includes(c.severity)
    );
    const missing = [];
    for (const t of data.templates || []) {
      if (t.archived || t.academicYearId !== academicYearId || !data.campuses?.some(
      ...
      try {
        ...
        missing.push(`${t.name}: ${e.message}`);
        continue;
      }
      for (const w of data.teachingWeeks || []) {
        if (w.archived || !w.isTeachingWeek || w.academicYearId !== academicYearId || !t.teachingWeeks.includes(w.weekNumber) || w.endDate < start || w.startDate > end)
          continue;
        const n2 = sessions.filter(
          (s2) => s2.teachingWeek === w.weekNumber && data.series?.find((x) => x.id === s2.seriesId)?.activityTemplateId === t.id
        ).length;
        if (n2 < t.weeklySessions)
          missing.push(
            `${t.name}, week ${w.weekNumber}: ${n2}/${t.weeklySessions} sessions`
          );
      }
    }
    return {
  ...
  function makePublication(data, scope, uuid, now2) {
    if (data.publication?.status !== "Approved" || data.publication.dataRevision !== data.dataRevision)
    ...
    const publication = {
      ...scope,
      id: uuid(),
      version: Math.max(0, ...(data.publications || []).map((p) => p.version)) + 1,
      status: "Published",
      publishedAt: now2,
      sessionCount: result.sessions.length,
      variantCount: new Set(
        result.sessions.map((s2) => s2.variantId).filter(Boolean)
      ).size,
      dataRevision: data.dataRevision
    };
    return { publication, data: JSON.parse(JSON.stringify(data)) };
  }
  function publishedVariantChanges(data, p, now2) {
    return (data.variants || []).filter((v) => {
      if (v.archived) return false;
      const series = data.series?.find((s2) => s2.id === v.seriesId);
      if (!series || series.archived || series.academicYearId !== p.academicYearId || !p.campusIds.includes(series.campusId))
        return false;
      ...
      return dates.length > 0 && dates.every((o) => o.date >= p.startDate && o.date <= p.endDate);
    }).map((v) => ({
      entity: "variants",
      record: {
        ...v,
